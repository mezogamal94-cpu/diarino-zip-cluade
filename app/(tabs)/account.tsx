...
import {
  View, Text, Pressable, ScrollView, StyleSheet, TextInput, Modal, Linking, Alert,
} from "react-native";
import * as ImagePicker from "expo-image-picker";
import { Image } from "expo-image";
import Svg, { Path, Rect, Circle } from "react-native-svg";
import { useProperties, useMyProperties } from "../../lib/hooks/useProperties";
...
type AccountTab = "ads" | "requests" | "lives" | "favorites";

// ↔ page-account / renderAccountContent() in app-viewer.html.
// myAds/myRequests/savedLives all start empty in the original too (a fresh
// account with nothing published yet) — real content only appears once the
// publish-listing / create-request / go-live flows are used, so the ads,
// "طلباتي", and "لايفات" tabs correctly show their empty states below until
// those flows exist. Favorites is fully real since it reads the shared store.
export default function AccountScreen() {
  ...
  useEffect(() => {
    if (tabParam && ["ads", "requests", "lives", "favorites"].includes(tabParam)) {
  ...
  const favProps = useMemo(() => properties.filter((p) => favoriteProperties.has(p.id)), [properties, favoriteProperties]);
  ...
  // ↔ handleAvatarClick() — now actually uploads to the "avatars" bucket
  // and persists the public URL onto the profile row, instead of only ever
  // holding a local file:// URI that vanished on the next app launch.
  async function pickAvatar() {
    ...
    try {
      ...
      const { error } = await supabase.storage.from("avatars").upload(path, blob, { contentType: `image/${ext}`, upsert: true });
    ...
    <View style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={styles.profileHeader}>
          ...
          <Text style={styles.handle}>@user_diartok</Text>
        </View>
          ...
          {activeTab === "lives" && (
            savedLives.length === 0 ? (
              <EmptyState
                ...
                onAction={() => router.push("/live/broadcast")}
              />
            ) : (
              <View style={styles.myAdsGrid}>
                {savedLives.map((live) => (
                  ...
                  >
                    {live.posterUrl ? (
                      <Image source={{ uri: live.posterUrl }} style={StyleSheet.absoluteFill} contentFit="cover" />
                    ) : (
